...
EXCEPTION_DEFINE(R_BAD_INPUT, NONE);
  EXCEPTION_DEFINE(R_UNEXPECTED, R_BAD_INPUT);
  EXCEPTION_DEFINE(R_BAD_FORMAT, R_BAD_INPUT);


unsigned crc32_c(const unsigned char *b, const unsigned char *e, unsigned x) {
  static const unsigned CRC32_C[256] = {
  ...
  while (b < e) x = (x << 8) ^ CRC32_C[((x >> 24) ^ *b++) & 0xFF];
